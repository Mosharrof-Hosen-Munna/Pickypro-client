...
const MySingleReview = ({
  ...
  const handleSaveReview = () => {
    axios
      .put(
        `https://pickypro-server.vercel.app/api/review/update/${review._id}`,
        {
          rating: ratingNumber,
      ...
      <div className="flex items-start  bg-white shadow-lg rounded-lg shadow-slate-200 p-4 my-8">
        ...
        <div className="md:ml-8 ml-3 w-full">
          <div className="flex items-center justify-between">
            ...
            <div className="hidden md:block items-center text-rose-600">
              <div>
                <StarRatings
                  rating={review.rating}
                  starRatedColor="purple"
            ...
            <div className="md:flex">
              ...
              <div
                ...
                className="text-lg mb-3 md:mb-0 py-1 px-2 bg-slate-200 rounded-lg  font-bold text-purple-700 cursor-pointer"
              >
                Edit
              </div>
              <div
                onClick={() => handleReviewDelete(review._id)}
                className="text-lg py-1 px-2 md:ml-8 bg-slate-200 rounded-lg font-bold text-red-600 cursor-pointer"
              >
                Delete
              </div>
            </div>
          ...
          <div className="mt-2">
            <h1 className="text-2xl font-semibold">
              For <span className="text-purple-700">{service.title}</span>{" "}
              service
            </h1>
            {service.description && (
              <SimilarServiceCard isShow={true} service={service}>
                <div>
                  <div className="hidden md:block">{service.description.slice(0, 200)} ...</div>
                  <div className=" md:hidden block">{service.description.slice(0, 50)} ...</div>
                </div>
              </SimilarServiceCard>
            )}
          </div>
